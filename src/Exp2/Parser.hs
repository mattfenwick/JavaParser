...
typeParams :: Parser e Token [TypeParameter]
typeParams = 
    op LessThan                     *> 
    sepBy1 typeParam (sep Comma)   <* 
    op GreaterThan
  where
    typeParam = pure TypeParameter  <*> 
                identifier          <*> 
                optional [] (key Kextends *> sepBy1 type' (op And))
...
typeArgs :: Parser e Token [TypeArgument]
typeArgs = 
    op LessThan                          *>
    sepBy1 typeArg (sep Comma)          <*
    reinterpretOperator -- in case the next token is an operator beginning with '>'
  where
    typeArg = wildcard <|> fmap TASimple type'
    wildcard = 
        op QuestionMark *> optional Wild ((key Kextends *> fmap WildExtends type') <|> 
                                          (key Ksuper   *> fmap WildSuper type'))
